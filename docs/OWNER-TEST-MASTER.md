...
| 21.5 | `pnpm e2e -- --grep console-door` | Only the console journey runs — « Running 7 tests using 1 worker » — seven steps (five since F-74: no console for a non-staffer, the two-factor wall, the door opening with the six-item menu, the second staffer minted from `/admin/staff`, the billing role's single menu item; two since F-77: the snapshot of a tenant with a real intake key whose token and secret never appear, and the read-only console pages) — and it ends green | ⬜ |
...
## ROUND 24 — The support call's screen, and what it can never show (F-77)

> The tenant snapshot you could only `GET` since ROUND 20 is a page now:
> **Instantané**, beside **Utilisation** on any tenant's console page. It is
> the screen a support person reads on the phone with a dealer — rooftops,
> intake keys, texting window, brand state, connectors, and what the
> platform itself is running. It is built so that it CANNOT show an intake
> key's token or secret: the page reads only fields the schema names, a guard
> scans its source for any generic render, a test injects a fake secret past
> the network and proves it never reaches the markup, and the browser suite
> mints a real key and checks its secret is nowhere on the page. You need the
> same platform super-admin account ROUND 20 needed. Nothing else moved.

| # | What to do | Expected | Status |
|---|---|---|---|
| 24.1 | Open **Locataires** → any dealer → its page | Two links right-aligned under the header: **Utilisation** and **Instantané**. The left menu is unchanged (no new item). Click **Instantané** | ⬜ |
| 24.2 | Read the **Succursales** table | One row per store: its name with the code beneath, the status, the timezone, the texting number (« — » when none), « Définies » / « Non définies » under **Heures**, one cell « n entrants · n sortants · n livrés » for the last 30 days, and « Aucun message en 30 jours » when nothing was sent or received. The column words are the SAME words you read in ROUND 23.1 under Réglages → Succursales — they are one string, shared. Under the table, the assistant sentence from the store form's hours grid and « Définies par le locataire sous Réglages → Succursales. » | ⬜ |
| 24.3 | Read **Sources d'admission** for a dealer who has an intake key (if none exists, create one as that dealer: its store's page → **Sources d'admission** → « Créer la clé », and copy the token and secret when they are shown — they are shown once) | One row per key: its name, « Webhook JSON générique », the STORE's name — or « Organisation » for a key created without a store, the dealer group's front door — « Active », « Jamais » under **Dernier prospect accepté**. Nowhere on the page — search it with Ctrl+F, then open view-source and search again — the token or the secret you copied. A revoked key reads « Révoquée le … » even if it was never switched off | ⬜ |
| 24.4 | Read **Automatisations** for a dealer who never opened Réglages → Automatisations, then for one who saved a window in ROUND 23.7 | First: exactly the sentence the dealer sees — « Aucune configuration enregistrée : les valeurs par défaut de la plateforme s'appliquent (…) » — and under it « Aucune dérogation par succursale ». Second: the window, the first-response exemption, the daily cap, under the same labels as the dealer's page; the dérogation line is still there and still says zero | ⬜ |
| 24.5 | Read **Image de marque** for a dealer who never touched the editor, then after that dealer saves an edit in the brand editor (ROUND 16) and again after publishing it | « Aucune image de marque »; after an edit that is not published, « Modifications non publiées » with « Version n » and « Jamais publiée »; after a publish, « Publiée », the version and « Publiée le … ». Read the caption: unpublished edits do not take the live brand down | ⬜ |
| 24.6 | Read the last card, **Plateforme — identique pour tous les locataires** (dashed border) | Three raw values — the SMS, e-mail and assistant transports — the same for every dealer you open (in dev, with the default `.env`: `log`, `log`, `off`; if you set a real transport, it reads that). The caption tells you « log » means no real send and « off » means the assistant does not run, and that none of it is changeable here | ⬜ |
...
## ROUND 28 — What each lender answered, and the one approval the deal follows (F-81)

> Your desking screen now has a ledger beneath the financing fields:
> **Soumissions aux prêteurs** — one card per submission (a lender re-shopped after a decline gets a second card),
> with the platform, what they answered (« Soumise », « Approuvée »,
> « Conditionnelle », « Refusée »), their buy and sell rates, the approved
> ceiling, the term, their quoted payment, conditions and an expiry. One
> button — **« Choisir cette approbation »** — makes the deal follow that
> offer: it writes the lender, the sell rate and the term onto the
> worksheet and the payment recomputes. Two warnings before you click
> anything: **a submission can never be deleted** (it is negotiation
> evidence — every field can be corrected (the chosen row's rate / term / lender only once it is unchosen), and a status can be moved back,
> but the card stays), and **choosing an approval rewrites the deal's rate,
> term and lender**. So walk this round on a THROWAWAY lead and deal — the
> last row says how to tidy up. Your registry already holds the real
> lenders these steps name (TD Auto Finance, Scotia Dealer Advantage « SDA »,
> iA Financial Group (Industrial Alliance) « iA »). One reading note: the « Soumise » / « Approuvée »
> badges below are the SUBMISSION's, inside the Soumissions aux prêteurs
> card — not the deal's stage of the same name on the lead page.

| # | What to do | Expected | Status |
|---|---|---|---|
| 28.1 | Create a throwaway prospect, open « Créer une transaction », desk a **Financement / Québec** deal (the defaults) — Prix de vente `30000`, Taux `4,99`, Terme `48`, no down payment — and read the bottom of the left column BEFORE saving; then save and reopen the deal from the lead page — its deal line's « Modifier » link (accessible name « Modifier la transaction — … ») | Before the save the card says only « Enregistrez la transaction pour consigner les réponses des prêteurs. » — no form. After the reopen it reads « Aucune soumission — consignez la première réponse d’un prêteur. » with the « Ajouter une soumission » form already open | ⬜ |
| 28.2 | Log two answers. First: Prêteur sollicité « TD Auto Finance », Plateforme « DealerTrack », Taux d’achat `5,99`, Taux de vente `6,99`, Terme approuvé `72`, Plafond approuvé `28000`, Paiement cité `650` → « Enregistrer la soumission ». Then « Ajouter une soumission »: « Scotia Dealer Advantage », « Manuelle », Taux de vente `8,49` only | Two cards, each « Soumise ». TD reads « Achat 5,99 % \| Vente 6,99 % \| Écart 1,00 % » (the spread is computed for display, never stored) and « 28 000,00 $ @ 6,99 % × 72 mois ». Scotia reads « Achat — \| Vente 8,49 % \| Écart — » — an untyped side is a dash, never « 0,00 % ». No star anywhere; the worksheet still says 4,99 / 48 / « Aucun prêteur » | ⬜ |
| 28.3 | « Modifier — TD Auto Finance » → Statut « Approuvée », Expiration = today + 30 days → « Enregistrer » | TD's badge is « Approuvée » with « Expire le … » and the sentence « Paiement cité par le prêteur : 650,00 $ — la feuille de calcul calcule celui de la transaction. » — the lender's number, captioned as theirs; the worksheet's own payment stays in Résultats. TD's « Choisir cette approbation » is live; Scotia's is greyed with « Seule une approbation peut être choisie. » | ⬜ |
| 28.4 | Click **« Choisir cette approbation — TD Auto Finance »**. Then type `5,50` into the worksheet's Taux, watch, type `6,99` back. Then « Enregistrer les modifications » and reopen the deal | The worksheet now holds 6,99 / 72 / « TD Auto Finance » and says « Modalités du prêteur appliquées à la feuille de calcul : 6,99 % sur 72 mois. »; TD wears ★ « Approbation choisie » and a warning chip « Le montant financé dépasse le plafond approuvé (28 000,00 $). » — 30 000 $ with Quebec taxes finances 34 492,50 $, more than TD's ceiling; it WARNS, it never refuses (re-desk or re-shop to resolve it). Typing 5,50 removes the applied line and shows « La feuille de calcul ne correspond plus à l’approbation choisie — choisissez-la de nouveau pour la réappliquer. »; 6,99 back removes the chip (the line does not come back — only the button writes it). After save + reopen the worksheet still says 6,99 / 72 / TD and TD still wears the star | ⬜ |
| 28.5 | « Ajouter une soumission »: « iA Financial Group (Industrial Alliance) », « CreditApp », Taux de vente `9,99`, Terme approuvé `60`, Conditions « Preuve de revenu » → save. « Modifier — iA… » → Statut « Conditionnelle » → save. Try its « Choisir cette approbation » button; tick « Conditions remplies » on the card; « Modifier — iA… » → Statut « Approuvée » → save; then « Choisir cette approbation — iA Financial Group (Industrial Alliance) » | The iA card reads « 9,99 % × 60 mois » (a term is shown even with no ceiling). While « Conditionnelle » its button is greyed: « Approuvez la soumission — conditions remplies — avant de la choisir. »; with the box ticked it is still greyed until the status is « Approuvée ». After the click the star MOVES: iA wears it, TD does not, exactly one in the card; the worksheet reads 9,99 / 60 / iA. Pipeline: the deal's card reads « Prêteur : iA » — its stage did not move | ⬜ |
| 28.6 | Decline the chosen one: « Modifier — iA… » → Statut « Refusée », Motif du refus « Revenu insuffisant » → save. Then « Modifier — iA… » → Statut « Soumise » → save | The star is gone and iA reads « Refusée » with its reason. The worksheet STILL names iA and 9,99 / 60 — those are now the worksheet's own inputs, and the deal never reads « Aucun prêteur » behind your back (edit them on the worksheet if they should change). Moving iA back to « Soumise » clears the reason (a reason belongs to a decline; the clearing is on the activity trail). Nothing offered to delete the card — nothing ever will | ⬜ |
| 28.7 | Choose TD again, then « Modifier — TD Auto Finance » and look at the form | TD wears the star and the worksheet holds 6,99 / 72 / TD again. In TD's form, Prêteur sollicité, Taux de vente and Terme approuvé are greyed with « Les modalités de l’approbation choisie sont verrouillées — modifiez la feuille de calcul, ou repassez la soumission à « Soumise » pour la corriger. » — the deal and its chosen offer cannot drift apart through this door; Plafond approuvé, Paiement cité, Expiration and Notes still edit | ⬜ |
| 28.8 | The bell. On the worksheet set « Vendu par » to one of your **Vendeurs** and save. Signed in as a DIFFERENT account (you, the owner), « Modifier — Scotia Dealer Advantage » → Statut « Approuvée » → save. Then sign in as that Vendeur and follow the bell | The Vendeur's bell reads « Approbation reçue : Scotia Dealer Advantage » and opens the deal's desking screen — where the card offers « Ajouter une soumission » and « Modifier — Scotia Dealer Advantage »: a Vendeur WRITES this ledger (the same deal:update that lets them desk the deal), unlike F-80's registry. YOUR bell has nothing new — the person who records the approval is never notified of their own click; approving a submission on a deal that names YOU as the seller rings nobody | ⬜ |
| 28.9 | Read-only, as an **Agent BDC**. Équipe → « Ajouter un membre »: name, a spare e-mail, UNTICK « Vendeur » (the form pre-ticks it) and tick « Agent BDC » → « Inviter »; copy the « Lien d’invitation » and accept it in a private window; open the throwaway deal's desking URL as that member | The roster row reads « Agent BDC » alone. On the desking screen the ledger shows every card from the steps above with « Vous pouvez consulter les soumissions ; votre rôle ne permet pas de les consigner. » — no « Choisir cette approbation », no « Modifier », no « Ajouter une soumission », no checkbox; conditions read as text. This claim is about the SUBMISSIONS card: the worksheet's own « Enregistrer les modifications » is not hidden by role today (that predates this slice; a refused save shows the generic save error). **Cleanup**: Équipe → « Retirer » on the test member | ⬜ |
| 28.10 | From `main-project`: `pnpm e2e -- --grep submissions` | « Running 6 tests using 1 worker » — born empty then TD and Scotia logged; TD approved with the captioned quote; the choice rewriting the worksheet, the ceiling warning, the re-save that holds; the conditional journey on iA and the one-star flip; the Agent BDC who reads and cannot write (zero write requests); the pipeline card naming iA — green in under a minute | ⬜ |
| 28.11 | **Cleanup** | The throwaway lead: mark it lost (« Marquer comme perdu ») — its deal and its submissions stay on file, as every ledger does. Leave TD Auto Finance, Scotia Dealer Advantage and iA active — they are real lenders you will use. The test member from 28.9 is retired in that row | ⬜ |

## ROUND 29 — Nobody from the old roster is named in this repository any more (F-82a)

> The legacy application's seed carried your twelve salespeople's real names
> beside their pay plans, and the names had spread into the plan documents,
> one specification, a form placeholder and one golden test. They are now
> « Vendeur 01 » … « Vendeur 12 » everywhere in this repository — the numbers
> beside them (rates, pads, the 60 000 $ tier, the 5 % overrides) did not
> move — and a guard fails the build if a name comes back. Nothing to click
> in the app this round: three commands and one file to open.

| # | What to do | Expected | Status |
|---|---|---|---|
| 29.1 | From `main-project`: `RLS_REQUIRED=1 REDIS_URL=redis://localhost:6381 npx vitest run apps/api/src/real-name-leak.test.ts` | « 4 passed » — the first test hands the guard a banned name in memory and expects it flagged (so a guard that stopped looking would fail here), the last proves the tree holds none | ⬜ |
| 29.2 | `git grep -ci "<given name> <surname>"` — any of the twelve, first name and surname together, inside the quotes; type it yourself, it is in no file for you to copy from. (A surname alone is not the test: two of the twelve are also your own and a persona's name, one is a piece of « Safari », and those are banned only as full names — D-083 (3)) | No output at all (zero files), for every one of the twelve. Try a second one if you like | ⬜ |
| 29.3 | Open `packages/core/src/money-math.test.ts` and find « commission engine » | The golden reads « Vendeur 03 deal with Vendeur 07 override », the tier test « Vendeur 10 » — and the numbers under them are the ones you know: 25 %, pad 150 000 ¢, gross for commission 550 000 ¢, commission 137 500 ¢, override 27 500 ¢, the tier flipping only above 6 000 000 ¢ | ⬜ |
| 29.4 | Put a name back on purpose: type one person's full name — given name and surname — at the end of `docs/PROJECT.md`, run 29.1 again, then delete the line. Both words: a surname that is also a first name in this project (yours, a persona's) is deliberately not banned on its own, D-083 (3), so six of the twelve surnames alone would stay green | Red: « docs/PROJECT.md:NNN:NN — a banned real name (… characters, sha256 …) » — the guard points at the line and never prints the name. Green again once the line is gone, and `git status` shows PROJECT.md unchanged | ⬜ |
| 29.5 | Two things only you can do — `docs/OWNER-ACTIONS.md` (2026-09-04) and `docs/OWNER-DECISIONS-PENDING.md` D-083 | The same scrub in your upstream `kia-tracker-specs` repository (and its seed script env-guarded or deleted), and your decision on rewriting this repository's history — the names remain in every commit before F-82a until you decide | ⬜ |

## ROUND 30 — Les dépenses d’un véhicule, et le coût que la feuille de calcul copie (F-82)

> A car's page now has a third block, **Dépenses du véhicule** — a ledger
> of what the car cost AFTER you bought it: one card per invoice
> (category, vendor, amount + taxes, date, invoice number), a status
> (« En attente » → « Approuvée » → « Payée », or « Refusée » / « Annulée »)
> and a receipt you attach as a PDF or a photo. Logging is lot work
> (anyone who can edit inventory); approving, refusing, paying and voiding
> is a manager's door (« Approuver, refuser, payer et annuler les dépenses
> des véhicules » — owner, GM and used-car manager by default) that also
> needs the cost view for that store. Two new rows join the cost strip:
> **Dépenses ajoutées** (approved + paid) and **Coût avec dépenses**. Read
> the caption under the second one before anything else — it is the whole
> point of this round: the ledger is a record and a report input —
> jamais un intrant de la feuille de calcul. Your « Coût total » does not
> move, and a new worksheet on the car copies the total cost, never the
> ledger's figure. An amount can never be edited: a wrong amount is voided and
> logged again. Nothing is ever deleted. Signed in as the owner account
> (hassan-test@1dealer.ca — every role) on « Groupe Hassan »: your one live
> car is A-1001 (Kia Mont-Laurier) — Acquisition 22 000,00 $ / Transport
> 0,00 $ / Reconditionnement 48 000,00 $ → Coût total 70 000,00 $. The
> round leaves two test lines on that car's ledger and one throwaway
> prospect; 30.11 says how to tidy up.

| # | What to do | Expected | Status |
|---|---|---|---|
| 30.1 | **Inventaire** → open A-1001 | Under the cost list, two new rows: « Dépenses ajoutées » 0,00 $ and « Coût avec dépenses » 70 000,00 $ — the same figure as « Coût total » — with the caption « Coût total plus les dépenses approuvées et payées du registre. La feuille de calcul copie le coût total, jamais ce montant. » Under the « Reconditionnement » input: « Les lignes de reconditionnement du registre ne modifient jamais ce champ. » Below the grid, the block **Dépenses du véhicule** reads « Aucune dépense — consignez la première facture. » with the « Consigner une dépense » form already open, its date set to today | ⬜ |
| 30.2 | Log the first invoice: Catégorie « Esthétique », Fournisseur `Lave-Auto Express`, Montant ($) `340`, Taxes ($) `50,92`, N° de facture `LAE-1042` → « Enregistrer la dépense » | One card « Esthétique — Lave-Auto Express » with the badge « En attente », today's date followed by « · Facture LAE-1042 », the money line « 340,00 $ + taxes 50,92 $ = 390,92 $ », and above the cards « En attente d’approbation : 390,92 $ ». The strip does not move: « Dépenses ajoutées » still 0,00 $, « Coût avec dépenses » still 70 000,00 $ (pending money is never in a sum) | ⬜ |
| 30.3 | On that card, « Approuver » (its accessible name is « Approuver — Lave-Auto Express ») | The badge reads « Approuvée »; the pending line is gone; « Dépenses ajoutées » 390,92 $ and « Coût avec dépenses » 70 390,92 $ — and « Coût total » is UNCHANGED at 70 000,00 $. The card now offers « Marquer payée », « Annuler la dépense » and « Joindre un reçu »; « Modifier » left with the pending state | ⬜ |
| 30.4 | « Marquer payée » on the same card | « Payée ». Both sums exactly as in 30.3 — paid counts like approved, nothing else moved. Only « Annuler la dépense » and « Joindre un reçu » remain | ⬜ |
| 30.5 | « Joindre un reçu » on the card → pick a PDF or a photo of an invoice (JPEG or PNG) | « Voir le reçu » appears on the card; clicking it opens the file in a new tab, served back after the server re-checks the file's hash. A second upload replaces the link, never the history | ⬜ |
| 30.6 | « Consigner une dépense » → Catégorie « Pièces », Fournisseur `Pièces Kia Laval`, Montant ($) `120`, no invoice → « Enregistrer la dépense » | A second card « Pièces — Pièces Kia Laval », « En attente », « 120,00 $ + taxes 0,00 $ = 120,00 $ »; the pending line reads « En attente d’approbation : 120,00 $ »; the two sums unchanged | ⬜ |
| 30.7 | « Modifier » on the Pièces Kia Laval card (a « Modifier » exists ONLY while a line is « En attente » — the Lave-Auto card has none) | The form reads « Modification — Pièces Kia Laval »; the amount and tax inputs are gone, replaced by « Montant fixé à la saisie — annulez la dépense et consignez-la de nouveau pour corriger le montant. » Type N° de facture `PKL-2041` → « Enregistrer la dépense »: the card gains « · Facture PKL-2041 » and still reads 120,00 $. Then « Refuser » on that card → « Refusée » — no buttons left on it, the pending line gone, both sums exactly as before (a refused line is in no sum) | ⬜ |
| 30.8 ⚠ THE FENCE | Create a throwaway prospect in **Kia Mont-Laurier** → « Créer une transaction » → in « Véhicule » pick « A-1001 — … » → read « Coût du véhicule », then save the deal | « Coût du véhicule » prefills `70000.00` — your Coût total, NOT the 70 390,92 $ the ledger shows. Save; back on A-1001 the page still reads « Coût total » 70 000,00 $ and « Coût avec dépenses » 70 390,92 $. This is the money ruling on the surface you desk on: the worksheet copies the total cost, never the ledger's figure, so front gross and commissions never move with an approval | ⬜ |
| 30.9 | Read-only, as an **Agent BDC** (28.9's recipe): Équipe → « Ajouter un membre », a spare e-mail, UNTICK « Vendeur », tick « Agent BDC » → « Inviter »; accept the « Lien d’invitation » in a private window; open A-1001's page as that member | Inside **Dépenses du véhicule**: both cards are there — headings and badges — with no amounts, no « Voir le reçu », no button, and the sentence « Vous pouvez consulter les dépenses ; votre rôle ne permet pas de les consigner. » The cost list shows no « Coût total » and neither new row (costs are masked for that role, and a masked page never shows a « 0,00 $ ») while « Type » still renders. This claim is about the Dépenses du véhicule block: the page's own « Reconditionnement » / « Prix affiché » form and its « Enregistrer » button are not hidden by role today (that predates this slice; a refused save shows the generic save error). **Cleanup**: Équipe → « Retirer » on the test member | ⬜ |
| 30.10 | From `main-project`: `pnpm e2e -- --grep expenses` | « Running 6 tests using 1 worker » — born empty; two lines logged; approve + pay with « Coût total » still 27 650,00 $ beside « Coût avec dépenses » 28 040,92 $; void through the two-click button and a PNG receipt; the desk that copies the triplet (27650.00, never 28040.92); the Agent BDC who reads everything and writes nothing (zero write requests) — green in under a minute | ⬜ |
| 30.11 | **Cleanup** | On the Lave-Auto Express card, « Annuler la dépense » → the same button now reads « Confirmer l’annulation » and has the focus → click it again → « Annulée », and « Dépenses ajoutées » is 0,00 $ again with « Coût avec dépenses » back to 70 000,00 $ (a paid line MAY be voided — the void is the record; money already out is the office's matter). The refused and the voided lines stay on file, excluded from every sum, as every ledger does. The throwaway prospect: « Marquer comme perdu » — its deal stays on file. A-1001 is still « Disponible » (desking writes no vehicle status). The test member was retired in 30.9 | ⬜ |

...
**BATCH-02 (owner enlarged it to 3-5 slices per D-032 — F-06 pipeline is MERGED; F-07 inventory next; F-08/F-09 proposed below):**
...
| F-08 | **Deal → delivery checklist**: mark a delivered deal's required steps (per-store configurable items, D-020) so "delivered" means something auditable. AHMAD half: checklist template per store + per-deal items + completion API. HUSSEIN half: checklist panel on the deal. | BOTH | BACKLOG | F-06, F-07 | Proposed 2026-07-25 under D-032's larger batch; the plan's next module after inventory (delivery/PDI gates). |
| F-09 | **Commissions on funded deals**: when a deal is funded, compute the salesperson's commission with the A-06 engine (pad-before-rate, tier by funded month, overrides) and show it. AHMAD half: pay-plan per membership + commission rows on funding. HUSSEIN half: commission view. | BOTH | BACKLOG | F-06 | Proposed 2026-07-25. The engine and its golden tests already exist (A-06); this wires them to real deals — the owner's 12 real pay plans become live data. |

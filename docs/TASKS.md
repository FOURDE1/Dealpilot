...
| F-14 | **White-label branding (Phase 2, ADR-018)**: the `tenant_branding` record, draft/published with a frozen publish snapshot, and the OKLCH + WCAG 2.2 colour engine in `packages/core` — contrast computed server-side and unreadable colours AUTO-FIXED rather than rejected, with every adjustment stored and reported. Plus the ADR-018 release-blocker guard: no hardcoded dealer branding in shipped source. AHMAD half only; the CSS variable injection and theme editor are HUSSEIN's. | AHMAD | DONE(2026-07-26: 528/528; colour maths golden-tested against WCAG's own worked values; brand-leak guard mutation-proven) — HUSSEIN: injection increment 1 (tenant name + radius, f92dae1) + increment 2 (focus ring, safe on CR-15's ring.*) + the theme editor (draft/publish/contrast, ca09d20; CR-16 defaults aligned bd58058). STILL TO COME: (a) the button/link FILL colours — need an app token role-split (fills→bg-primary, text→a new text-primary-ink for the 23 link usages, + a --primary-hover-foreground token & Button change), then inject fills/foregrounds/hover for both themes with a both-theme contrast e2e; (b) brand ASSET display (logo/favicon) once built — **SECURITY CONVENTION (Ahmad's F-13c/F-14b audit, docs/SECURITY.md): a tenant SVG logo can carry script; render it as `<img src>` (neutralised by the response CSP+sandbox+nosniff), NEVER inline the SVG markup into the DOM.** | F-13c | Billing, custom domains and asset upload are the rest of Phase 2 and need paid AWS/Stripe — deliberately not started while the owner's instruction is that no paid resource is provisioned. |
...
| F-15 | **Compliance engine (Phase 3, CASL/CRTC/Law 25)**: consent ledger (append-only, trigger-enforced), suppression list, internal do-not-call, per-tenant quiet-hours config, append-only send decisions; the OKLCH of this slice is `evaluateSend` — one pure function the send layer calls and nothing else. Recipient-local quiet hours with real timezone arithmetic, CRTC voice windows, consent expiry in calendar months, ADAD express-consent rule, national-list staleness fail-closed. | AHMAD | DONE(2026-07-27: 632/632; 39 adversarial core cases from §12's red-team list + 20 API cases incl. a live cross-tenant RLS probe as the app role) | — | The plan calls this binary: "a CASL violation costs up to $10M". Every spec ambiguity resolves to BLOCKED, and the eight genuine gaps are D-042 rather than guesses. |
| F-15a | **Self-initiated enquiry consent (D-042 #1, owner answered 2026-07-27)**: a walk-in or phone lead who gives their number gets an implied-inquiry basis written in the same transaction as the lead — conversational only, six months, no referrals, no automated-call permission. | AHMAD | DONE(2026-07-27: 640/640) | F-15 | Before this every walk-in and phone lead was permanently unmessageable: the customer gives you their number at the desk and the system refuses to text them. |
| F-16 | **AI safety layer (Phase 3, conversation-engine.md §10–§11)**: the deterministic `outboundGuard` post-filter (currency in both Canadian forms, rates, approval promises, delivery promises, invented stock numbers) and the prompt-injection defence (spotlight wrapping, wrapper defanging, invisible-character stripping, field ceilings). Ships BEFORE any model does. | AHMAD | DONE(2026-07-27: 659/659; 19 adversarial cases) | F-15 | The safety layers are what make a jailbroken model harmless rather than merely unlikely — and they are fully testable with no API key, so they should not wait for one. Found a real trap on the way: JavaScript's `\b` never matches after an accented letter, so every FRENCH approval promise walked through the first version. |
| F-17 | **Intake connector framework (Phase 3, ADR-005 amended)**: a lead source is a DEFINITION — field paths, the source name, and what that form's consent box actually granted — not a code path. Phone normalised to one E.164 form, deterministic dedupe key, and the consent the form collected written in the same transaction as the lead. Intake keys name their connector. | AHMAD | DONE(2026-07-27: 679/679) | F-15 | Webhook leads previously arrived with NO consent basis at all: the form collected permission and the system discarded it, so every intake lead was unmessageable. Same gap D-042 #1 closed for walk-ins. ADF/XML is NOT built — it needs an XML parser, and a new dependency is the owner's call. |
| F-17b | **ADF/XML leads (AutoTrader.ca, Kijiji Autos)**: parser hardened against entity expansion with a size ceiling, name-part and single-name handling, mobile-preferred phone selection, buy-vs-trade-in vehicle selection, and NO invented consent. Redis added to compose for the queue and console. | AHMAD | DONE(2026-07-27: 690/690) | F-17 | Owner approved the dependency; pinned to 4.5.7 rather than 5.x — see D-043. |

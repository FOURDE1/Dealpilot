...
| A-04 | packages/db + local Docker Postgres (dev) + first migration (tenants/orgs/stores/users + RLS); staging = RDS via IaC | AHMAD | CLAIMED(AHMAD, 2026-07-24) | A-01, A-03 | RDS over Supabase (D-013); staging RDS provisioned with the A-07 IaC baseline (owner approval before any AWS apply); dev needs no cloud resources |

...
| A-02 | CI pipeline: lint/typecheck/test on push to develop/main, frozen lockfile | AHMAD | BLOCKED(needs GitHub Actions billing unlock from OWNER) | A-01 | Workflow MERGED to develop (125c900, D-026): push-triggered (shared + agent branches), SHA-pinned, ephemeral PG:5434, db:reset from zero, RLS_REQUIRED=1 + --passWithNoTests=false. Local gate green (22/22 build, lint 0, 108/108 tests). Live run attempt died pre-start: "account is locked due to a billing issue" — github.com Settings→Billing. After unlock: any push should go green, then AHMAD runs a red-probe branch to prove failures fail → flip to DONE |
...
| H-03 | apps/web shell: routing, layout, auth screens against A-05 contract | HUSSEIN | IN-PROGRESS(increment 1 merged: 734e5f8) | A-03, A-05, H-02 | Shell + routing + guards + auth screens merged (builds green, 5 guard tests, login screenshot-verified). Remaining for DoD: live sign-in round-trip + Playwright e2e — **needs Docker** (not installed on this laptop; run on desktop or install here) |

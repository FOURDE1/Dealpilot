...
| F-11b | **Dispatch paperwork**: the driver dispatch email + `driver_companies` roster, pickup/delivery addresses, cash-to-collect, special instructions, the wet-ink booking gate, customer ETA notification, `status_updates` feed, typed depart/arrive/eta sub-routes. | AHMAD | BACKLOG | F-11 | Deliberately split out. F-11 shipped the scheduling core — the rule, the fleet, conflict detection and the resource lifecycle — and the commit says so rather than claiming all of dispatch-transport.md. |

...
| Log what a car cost after purchase, and approve it | On a car's page (**Inventaire → the car**), the **Dépenses du véhicule** block — one card per invoice: category (twelve codes, from « Sécurité / PDI » to « Autre »), vendor, amount + taxes, date, invoice number, a status badge (« En attente », « Approuvée », « Payée », « Refusée », « Annulée ») and a receipt (PDF, JPEG or PNG, its hash re-checked on every download) — over `GET`/`POST /api/v1/vehicles/:id/expenses`, `PATCH /api/v1/expenses/:id` and `POST`/`GET /api/v1/expenses/:id/receipt`. Logging, editing a pending line and attaching a receipt run under the inventory-editing authority (`vehicle:update`: owner, GM, sales manager, used-car manager, wholesale manager by default); approving, refusing, paying and voiding run under « Approuver, refuser, payer et annuler les dépenses des véhicules » (`expense:approve`: owner, GM, used-car manager by default) AND require the cost view for the car's store — you cannot approve a number you cannot read. Every member reads the cards; amounts, receipts and the two sums are absent for a role without « Voir les coûts des véhicules ». An amount is fixed at entry: void the line and log it again to correct it; nothing is ever deleted. The cost list gains « Dépenses ajoutées » (approved + paid) and « Coût avec dépenses », captioned: the worksheet copies the total cost — never a desk input — so « Coût total », « Coût du véhicule », gross and commissions never move with an approval (F-82) |

...
- **A local run is not CI.** Check the actual CI verdict before reporting a push as
  green (2026-07-26: three red pushes went unnoticed behind a passing local gate).
- **Never edit an applied migration.** CI rebuilds from zero and cannot catch it;
  every database with history will refuse to upgrade. Write a forward migration, and
  prove the chain with `db:migrate` against a database that already has data.

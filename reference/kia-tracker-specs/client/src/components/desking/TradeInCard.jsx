  ...
  return (
    <div className="border border-gray-200 rounded-lg p-3 bg-gray-50">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        ...
        <Field label={t('desking.model')}><TextInput value={trade.model} onChange={(x) => onChange({ model: x })} /></Field>
        <Field label={t('desking.mileage')}><NumberInput value={trade.mileage} onChange={(x) => onChange({ mileage: x })} suffix="km" /></Field>
        <Field label="VIN" className="md:col-span-2"><TextInput value={trade.vin} onChange={(x) => onChange({ vin: x })} /></Field>
        ...
        <Field label={t('desking.tradeLien')}><MoneyInput value={trade.lien} onChange={(x) => onChange({ lien: x })} /></Field>
      </div>
...
export default function TradeInCard({ state, setTrade, addTrade, removeTrade }) {
  const { t } = useTranslation();
  ...
  return (
    <SectionCard
      ...
      icon={Repeat}
      right={trades.length < 2 ? (
        <button onClick={addTrade} className="inline-flex items-center gap-1 px-2 py-1 text-xs bg-white/20 hover:bg-white/30 rounded">
          <Plus size={12} /> {t('desking.add2ndTrade')}
        </button>
      ) : null}
    >
